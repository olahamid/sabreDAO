...
ifeq ($(findstring --network arbitrum,$(ARGS)),--network arbitrum)
	NETWORK_ARGS := --rpc-url $(ARB_RPC_URL) --private-key $(ARB_SAPOLIA_PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif

deployToken:
	@forge script script/deploySabreDAO.s.sol:deploySabreDAO $(NETWORK_ARGS)

deployEngine:
	@forge script script/deploySareDAOEngine.s.sol:deploySabreDAOEngine $(NETWORK_ARGS)
